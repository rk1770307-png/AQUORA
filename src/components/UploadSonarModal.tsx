...
export const UploadSonarModal: React.FC<UploadSonarModalProps> = ({
  ...
  const handleStartInference = async () => {
    ...
    try {
      ...
      setProcessingStage('Executing YOLOv8 inference & acoustic contour proposals...');
      ...
      // Use the authentic report title generated by the backend AI if available
      const finalReportTitle = response.analysis_report?.report_title 
      ...
      const syntheticDataset: PresetDataset = {
        ...
        description: response.analysis_report?.dominant_anomaly
          ? `${response.analysis_report.dominant_anomaly} Survey (${response.total_found} hazards catalogued)`
          : `Backend Pipeline: ${response.pipeline} (${response.total_found} hazards catalogued)`,
        imageUrl: previewUrl || '',
        metadata: {
          auvId,
          vesselName,
        ...
        hazards: response.detections,
        speckleNoiseLevel: 'Medium'
      };

      onDetectionsSuccess(selectedFile, syntheticDataset, response.detections, response);
      onClose();
    } catch (err: any) {
      console.error('Inference pipeline error:', err);
      setErrorMsg(err?.message || 'Error processing sonar image on backend.');
    } finally {
            ...
            <div className="bg-cyan-950/40 border border-cyan-500/40 p-3.5 rounded-xl flex items-center gap-3 text-cyan-300 text-xs animate-pulse">
              ...
              <div>
                ...
                <p className="text-[10px] text-cyan-400/70 font-mono mt-0.5">
                  Connecting to Python Flask Backend (YOLOv8 + OpenCV Speckle & Shadow Trigonometry)
                </p>
              </div>
            </div>
          )}
